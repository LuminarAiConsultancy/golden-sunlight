...
**Check that the terrain calculation is deterministic.** Running `python diagnostics.py` twice regenerates `skyline_full.csv`, `skyline_fine_120_150.csv` and `skyline_fine_200_240.csv` byte for byte identically. This was performed on 6 September 2026 and `git status` reported no change to any of the three. The same run recorded a SHA-256 for each of the four elevation tiles and found zero voids in any of them, scanning all 51,868,804 raw tile samples, of which 51,840,000 form the working mosaic after the shared tile edges are trimmed. The tile hashes and void counts are in `diagnostics-run-2026-09-06.txt`.

**Read the commit history.** It is a record of what changed and what each change cost. Commit messages carry before and after figures where numbers moved, including a case where a 1.6 metre inconsistency between two modules shifted seventeen times by one minute each, and the replacement of an off-repository script with civildawn.py, which moved the count of mornings dark at 8:30 under permanent Mountain Daylight from 87 to 89 and three related counts with it. Both commits state what changed, by how much, and what did not move. The history is the audit trail; the code alone would not tell you that.
...
**The terrain half has not been checked against an independent measurement of the ridge, but it is no longer unexamined.** Two things were tested on 6 September 2026. Stability: tracing the skyline to 20, 40 and 60 kilometres gives identical results in every row, so the profile is not an artifact of where the trace stops, though 10 kilometres is too short and moves the solstice sunrise six minutes earlier. Sampling: measured against rays ten times finer, the half-degree spacing differs by 0.023 degrees RMS on the morning arc and 0.018 on the afternoon, and the planar ray approximation differs from a true geodesic by at most 0.120 degrees over the arcs the sun actually uses. A quarter of a degree of skyline error moves the solstice sunrise by about a minute, so both are comfortably inside the noise. What remains unchecked is accuracy rather than stability: no computed ridge angle has been compared against an independently known one. Closing that needs either surveyed horizon data or a timed observation of the sun clearing the ridge from the observation point. Until then the ridge angles are internally consistent and externally unverified.
...
The four tiles covering this area are N50W117, N50W118, N51W117 and N51W118, each 25,934,402 bytes. The first sixteen hex characters of a SHA-256 for each, as retrieved on 6 September 2026, are recorded in diagnostics-run-2026-09-06.txt alongside the void scan, so a reader who downloads the same tiles can confirm they are working from the same bytes by comparing the leading characters of their own digest. That file is the verbatim output of a diagnostics.py run, committed unedited.
